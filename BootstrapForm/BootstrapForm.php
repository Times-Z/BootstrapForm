...
class BootstrapForm extends Form {
     ...
     * @return string
     */
    public function text(string $name, string $label, string $placeholder = '', bool $required = false, $value = false, bool $disabled = false, $pattern = false) :string {
        $label = ($label !== '') ? '<label for="' . $name . '">' . $label . '</label>' : '';
        $value = ($value !== false) ? $value : $this->__getValue($name);
        $required = ($required !== false) ? 'required' : '';
        $disabled = ($disabled !== false) ? 'disabled' : '';
        $pattern = ($pattern !== false) ? 'pattern="' . $pattern .'"' : '';
